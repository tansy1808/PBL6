    ...
    public class AuthService : IAuthService
    {
        ...
        public ViewBag Change(int id, ChangePass changePass)
        {
            ...
            if (changePass.NewPassword == changePass.RePassword)
            {
                ...
                var password = Encoding.UTF8.GetBytes(changePass.NewPassword);
                pass.PasswordHash = hmac1.ComputeHash(password);
                pass.PasswordSalt = hmac1.Key;
        ...
        public ViewBag Register(AuthUserDTO authUserDto)
        {
            ...
            if (_userReponsitory.GetUserAnyEmail(authUserDto.Email))
            {
                var view3 = new ViewBag()
                {
                    Status = "Error",
        ...
        public MemberAPI GetUserAll(int page, int size)
        {
            ...
            var data = users.Skip((page - 1) * size).Take(size).ToList();
        ...
        public ViewBag ChangePass(string email, string newpass, string repass)
        {
            var pass = _userReponsitory.GetUserByEmail(email);
            using var hmac = new HMACSHA512();
            var password = Encoding.UTF8.GetBytes(newpass);
            if (newpass == repass)
            {
                
                pass.PasswordHash = hmac.ComputeHash(password);
