    ...
    public class ProductService : IProductService
    {
        ...
        public ViewProductFeedDTO AddProductFeed(ProductFeedDTO productFeedDTOs)
        {
            ...
            if (user != null)
            {
                ...
                feed.FeedDate = DateTime.Now.AddHours(8);
                ...
                if (pro != null)
                {
                    double st = (productFeedDTOs.star + pro.Feedback) / 2;
                    pro.Feedback = (int)st;
        ...
        public ViewProductDTO AddProduct(ProductDTO productDTO)
        {
            ...
            if (pro != null)
            {
                ...
                products.DateCreate = date.AddHours(8);
        ...
        {
            var pros = _productReponsitory.GetProductsbyName(name).OrderBy(c => c.Price*(1-c.Discount));
            var proprice = pros.Where(c => c.Price*(1-c.Discount) > st).Where(c => c.Price*(1-c.Discount) < end);
            var list = new List<ProductView>();
        ...
        public CategoryAPI GetProductByPrice(int categoryId, int st, int end, int page, int size)
        {
            var pros = from s in _context.Products.Where(c => c.IdCate == categoryId)
                       orderby s.Price*(1-s.Discount)
                       select s;
            var proprice = pros.Where(c => c.Price > st).Where(c => c.Price * (1 - c.Discount) < end);
            var list = new List<ProductView>();
            var productPage = new ProductPage();
            var view = new CategoryAPI();
            if (page != 0 && size != 0)
